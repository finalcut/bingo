  ...
  return <main className="room-layout">
    ...
    <aside className="side-panel">
      ...
      <div className="host-controls">
        ...
        {state.isHost && state.phase === 'lobby' && <label>Game mode<select aria-label="Game mode" value={state.mode.id} onChange={(event) => client.send({ type: 'setMode', modeId: event.target.value as Bingo75ModeId })}>{Object.values(bingo75Modes).map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.name}</option>)}</select></label>}
